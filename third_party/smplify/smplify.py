...
from .customloss import (camera_fitting_loss_3d, body_fitting_loss_3d,
                         temporal_rotation_loss, floor_penetration_loss)
...
class SMPLify3D():
    ...
    def __init__(self,
        ...
        self.temporal_pose_weight = temporal_pose_weight
    ...
    def __call__(self, init_pose, init_betas, init_cam_t, j3d, conf_3d=1.0, seq_ind=0):
        ...
        if self.shared_shape:
            # One person has one shape; independent shapes can shrink/stretch
            # the body from frame to frame to explain pose errors.
       ...
       # -------------Step 1: Optimize camera translation and body orientation--------
        ...
        if self.use_lbfgs:
            ...
            for i in range(self.num_iters):
                def closure():
                    ...
                    loss = loss + temporal_rotation_loss(
                        body_pose, global_orient, self.temporal_pose_weight)
                    loss = loss + floor_penetration_loss(
                        model_vertices, camera_translation, self.floor_height)
                    loss.backward()
                    return loss
        ...
        with torch.no_grad():
            ...
            final_loss = body_fitting_loss_3d(body_pose, preserve_pose, betas, model_joints[:, self.smpl_index], camera_translation,
                                              ...
                                              search_tree=search_tree,  pen_distance=pen_distance,  filter_faces=filter_faces)
            final_loss = final_loss + temporal_rotation_loss(
                body_pose, global_orient, self.temporal_pose_weight)
            final_loss = final_loss + floor_penetration_loss(
                model_vertices, camera_translation, self.floor_height)

        vertices = smpl_output.vertices.detach()
        joints = smpl_output.joints.detach()
        pose = torch.cat([global_orient, body_pose], dim=-1).detach()
        betas = betas.detach().expand(len(pose), -1)
